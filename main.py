...
def rentabilidade(message):
        ...
        bot.register_next_step_handler(message, lambda msg: ask_rentabilidade_final(msg))

def ask_rentabilidade_final(initial_message):
    data_inicial = initial_message.text
    if verificar_saida(data_inicial, initial_message):
        return
    try:
        datetime.datetime.strptime(data_inicial, "%d/%m/%Y")
    except ValueError:
        bot.reply_to(initial_message, "Data inválida. Tente novamente (DD/MM/AAAA) ou digite 'sair' para cancelar.")
        bot.register_next_step_handler(initial_message, ask_rentabilidade_final)
        return

    bot.reply_to(initial_message, "Digite a data final (DD/MM/AAAA):")
    bot.register_next_step_handler(initial_message, lambda msg: ask_rentabilidade_loja(initial_message, data_inicial, msg.text))

def ask_rentabilidade_loja(initial_message, data_inicial, data_final):
    if verificar_saida(data_final, initial_message):
        return
    try:
    ...
    bot.reply_to(initial_message, "Digite o número da loja:")
    bot.register_next_step_handler(initial_message, lambda msg: send_to_rentabilidade(initial_message, data_inicial, data_final, msg.text))

def send_to_rentabilidade(message, data_inicial, data_final, loja):
    if verificar_saida(loja, message):
        return
    if not loja.isdigit():
        bot.reply_to(message, "Número da loja inválido. Tente novamente ou digite 'sair' para cancelar.")
        bot.register_next_step_handler(message, lambda msg: send_to_rentabilidade(message, data_inicial, data_final, msg.text))
        return
    gerar_rentabilidade(message, data_inicial, data_final, loja)
...
def ask_tipo_consulta(user_message, original_message):
    ...
    bot.register_next_step_handler(original_message, lambda msg: get_lojas(original_message, usuario, msg.text))

data_atual = datetime.datetime.now()

def get_lojas(message, usuario, tipo):
    ...
    for loja in lojas:
        bot.reply_to(message, consultar_notas(inicial, final, loja))
...
def gerar_rentabilidade(message, inic, fim, loja):
    inic = tratar_data(inic)
    fim = tratar_data(fim)
    
    consultar_rentabilidade(inic, fim, loja)
    csv_para_xlsx(f"data/rentabilidade_{loja}_{inic}_{fim}.csv", f"rentabilidade_{loja}_{inic}_{fim}.xlsx", colunas_desejadas)
    
    bot.send_document(message.chat.id, open(f"rentabilidade_{loja}_{inic}_{fim}.xlsx", 'rb'))
